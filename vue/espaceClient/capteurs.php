...
?>
    <section id="capteur">
        ...
        <div id="listCapteurs" class="theme">
            <div>
                <h2>Mes Capteurs</h2>
                <div id="placement">
                <ul>
                    ...
                    <?php foreach ($arrayCapteurs as $key => $value) { ?>
                        <li class="capteur" id="list<?= $value['ID']; ?>" onclick="displayCross('isSet<?php echo $value['ID']; ?>','<?=  $value['ID'] ?>')"><h3><?php echo $value['nom'] ?></h3><span class="crossCapteur" id="<?php echo $value['ID'] ?>"></span>
                            <ul>
                                <?php if ($value['type'] == 'temperature') { ?>
                                <img src="/vue/style/images/thermometer.png" alt="thermo">
                                <?php } else if ($value['type'] == 'humidite') { ?>
                                <img src="/vue/style/images/humidity.png" alt="hum">
                                    <?php } ?>
